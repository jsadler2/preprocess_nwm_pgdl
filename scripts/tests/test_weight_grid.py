...
def test_resize_individual_cols():
    all_cols = wt.get_all_col_or_idx(df_list, 'col')
    resized = wt.resize_individual_df_cols(all_cols, df1)
    assert resized.columns.equals(combined_cols)
    assert resized.index.equals(df1.index)
